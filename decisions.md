...
| D-007 | 2026-09-07 | WordPress.org Directory Publishing & Dual-Channel Distribution Architecture | accepted | WordPress.org Guideline #17 forbids leading trademarks (renamed to "Email Guard for Fluent Forms"); Guideline #7 forbids third-party updaters in directory builds (release guard WPORG_RELEASE added). Established wheels 10up/action-wordpress-plugin-deploy and WordPress/plugin-check-action integrated | `readme.txt`, `.distignore`, `assets/`, `wporg_preflight_check.mjs` | - |
| D-008 | 2026-09-09 | Zero External Offloading & Bundled Offline Datasets | accepted | WordPress.org Plugin Check forbids fetching remote content (raw.githubusercontent.com) and unconsented WP-Cron tracking. Bundling 8,742 domains directly in data/disposable_domains.json guarantees zero external network dependencies, privacy compliance, and instant offline lookup | `data/disposable_domains.json`, `tests/test_email_guard_contract.py` | D-003 (partially: remote sync replaced by offline bundle) |
